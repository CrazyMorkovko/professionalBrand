<?php
include "header.template.php";
?>
<section class="slider">
    <div class="container slider-content">
        <div class="slider-text">
            <h2>THE BRAND</h2>
            <h4>OF LUXERIOUS <span class="pink">FASHION</span></h4>
        </div>
    </div>
</section>
<section class="sale">
    <div class="container sale-flex">
        <div class="sale-left">
            <div class="sale-big">
                <div class="sale-text">
                    <h4>Hot deal</h4>
                    <h2 class="pink">for men</h2>
                </div>
            </div>
            <div class="sale-small">
                <div class="sale-text">
                    <h4>luxirous & trendy</h4>
                    <h2 class="pink">accesories</h2>
                </div>
            </div>
        </div>
        <div class="sale-right">
            <div class="sale-small">
                <div class="sale-text">
                    <h4>30% offer</h4>
                    <h2 class="pink">women</h2>
                </div>
            </div>
            <div class="sale-big">
                <div class="sale-text">
                    <h4>new arrivals</h4>
                    <h2 class="pink">for kids</h2>
                </div>
            </div>
        </div>
    </div>
</section>
<section class="items-text">
    <div class="container">
        <h2>Fetured Items</h2>
        <p>Shop for items based on what we featured in this week</p>
    </div>
</section>
<section class="items">
    <div class="container">
        <div class="product-flex">
            <?php foreach ($products as $product):?>
            <div class="product">
                <a href="/?page=product-page&id=<?=$product['id']?>" class="product-card">
                    <div class="add-to-card-block">
                        <img src="/uploads/products/<?=$product['image']?>" alt="product" class="product-img">
                        <button class="add-to-card-button btn">
                            <i class="icon icon-cart"></i>
                            Add to Cart
                        </button>
                    </div>
                    <p class="titles"><?=$product['name']?></p>
                    <p class="pink price">$<?=$product['price']?></p>
                </a>
            </div>
            <?php endforeach;?>
        </div>
    </div>
</section>
<div class="container text-center">
    <a href="#" class="pink-button">Browse All Product <i class="fas fa-long-arrow-alt-right"></i></a>
</div>
<section class="banner">
    <div class="container">
        <div class="bannerContent">
            <div class="bannerImg">
                <img src="/img/Banner.png" alt="banner">
            </div>
            <div class="discounts">
                <div class="discountBlock">
                    <div class="iconDiscount">
                        <i class="icon icon-forma1"></i>
                    </div>
                    <div class="discount">
                        <h3>Free Delivery</h3>
                        <p>
                            Worldwide delivery on all. Authorit tively morph next-generation innov tion with extensive
                            models.
                        </p>
                    </div>
                    <div class="clearfix"></div>
                </div>
                <div class="discountBlock">
                    <div class="iconDiscount">
                        <i class="icon icon-forma2"></i>
                    </div>
                    <div class="discount">
                        <h3>Sales & discounts</h3>
                        <p>
                            Worldwide delivery on all. Authorit tively morph next-generation innov tion with extensive
                            models.
                        </p>
                    </div>
                    <div class="clearfix"></div>
                </div>
                <div class="discountBlock">
                    <div class="iconDiscount">
                        <i class="icon icon-forma3"></i>
                    </div>
                    <div class="discount">
                        <h3>Quality assurance</h3>
                        <p>
                            Worldwide delivery on all. Authorit tively morph next-generation innov tion with extensive
                            models.
                        </p>
                    </div>
                    <div class="clearfix"></div>
                </div>
            </div>
            <div class="clearfix"></div>
        </div>
    </div>
</section>
<?php
include "footer.template.php";
?>
